...
def temporary_settings_override(settings_override: dict):
    ...
    try:
        # 应用覆盖设置
        if "api_key" in settings_override:
            ...
            val = settings_override["api_key"]
            if val:
                current_app.config["GOOGLE_API_KEY"] = val
        ...
        if "api_base_url" in settings_override:
            ...
            val = settings_override["api_base_url"]
            if val:
                current_app.config["GOOGLE_API_BASE"] = val
                current_app.config["OPENAI_API_BASE"] = val
            else:
                current_app.config.pop("GOOGLE_API_BASE", None)
                current_app.config.pop("OPENAI_API_BASE", None)

        if "ai_provider_format" in settings_override:
            original_values["AI_PROVIDER_FORMAT"] = current_app.config.get("AI_PROVIDER_FORMAT")
            val = settings_override["ai_provider_format"]
            if val:
                current_app.config["AI_PROVIDER_FORMAT"] = val
            else:
                current_app.config.pop("AI_PROVIDER_FORMAT", None)

        if "text_model" in settings_override:
            original_values["TEXT_MODEL"] = current_app.config.get("TEXT_MODEL")
            val = settings_override["text_model"]
            if val:
                current_app.config["TEXT_MODEL"] = val
            else:
        ...
        if "image_model" in settings_override:
            ...
            val = settings_override["image_model"]
            if val:
                current_app.config["IMAGE_MODEL"] = val
            else:
        ...
        if "image_caption_model" in settings_override:
            ...
            val = settings_override["image_caption_model"]
            if val:
                current_app.config["IMAGE_CAPTION_MODEL"] = val
            else:
        ...
        for model_type in ('text', 'image', 'image_caption'):
            ...
            if key_field in settings_override:
                ...
                original_values[config_key] = current_app.config.get(config_key)
                val = settings_override[key_field]
                if val:
                    current_app.config[config_key] = val
                else:
                    current_app.config.pop(config_key, None)
            if base_field in settings_override:
                ...
                original_values[config_key] = current_app.config.get(config_key)
                val = settings_override[base_field]
                if val:
                    current_app.config[config_key] = val
                else:
                    current_app.config.pop(config_key, None)
...
def get_active_config():
    ...
    return success_response({
        ...
        "text_model_source": current_app.config.get("TEXT_MODEL_SOURCE"),
        "text_api_base": current_app.config.get("TEXT_API_BASE"),
        "image_model": current_app.config.get("IMAGE_MODEL"),
        "image_model_source": current_app.config.get("IMAGE_MODEL_SOURCE"),
        "image_api_base": current_app.config.get("IMAGE_API_BASE"),
        "openai_image_api_protocol": current_app.config.get("OPENAI_IMAGE_API_PROTOCOL"),
        "output_language": current_app.config.get("OUTPUT_LANGUAGE"),
        "image_caption_model": current_app.config.get("IMAGE_CAPTION_MODEL"),
        "image_caption_model_source": current_app.config.get("IMAGE_CAPTION_MODEL_SOURCE"),
        "image_caption_api_base": current_app.config.get("IMAGE_CAPTION_API_BASE"),
    })
