...
class FileParserService:
    ...
    def parse_file(self, file_path: str, filename: str) -> tuple[Optional[str], Optional[str], Optional[str], Optional[str], int]:
        ...
        try:
            ...
            # If MinerU token is not configured or is a placeholder, fall back to markitdown
            _token = (self.mineru_token or '').strip()
            _invalid_tokens = {'', 'your-mineru-token', 'your-token', 'placeholder'}
            if not _token or _token.lower() in _invalid_tokens:
                logger.info(f"MinerU token not configured, using markitdown for {filename}...")
                return self._parse_spreadsheet_file(file_path, filename)
