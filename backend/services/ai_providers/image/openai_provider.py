...
class OpenAIImageProvider(ImageProvider):
    ...
    def __init__(self, api_key: str, api_base: str = None, model: str = "gpt-image-2", image_api_protocol: str = 'auto'):
        ...
        self.api_base = normalized_base or ""
        self.model = model
    ...
    def _should_retry_with_chat_fallback(self, error: Exception) -> bool:
        """Return True when auto mode should retry via chat.completions."""
        message = str(error).lower()
        return any(token in message for token in (
            'cannot decode raw string as image',
            '<!doctype html',
            '<html',
            'no valid multimodal response received',
        ))
        ...
        try:
            ...
            if use_images_api:
                try:
                    ...
                    if self.image_api_protocol == 'auto' and self._should_retry_with_chat_fallback(images_error):
                        logger.warning(
                            "OpenAI images API returned a non-image or unsupported response for model=%s; "
                            "retrying via chat.completions fallback. error=%s",
                            self.model,
                            images_error,
                        )
                    else:
                        raise
            elif self.image_api_protocol not in ('auto', 'chat'):
                raise ValueError(f"Unsupported image_api_protocol: {self.image_api_protocol}")
