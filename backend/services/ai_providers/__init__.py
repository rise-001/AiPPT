...
def _build_provider_config() -> Dict[str, Any]:
    ...
    if fmt == 'openai':
        ...
        cfg['api_base'] = normalize_openai_base_url(_resolve_setting('OPENAI_API_BASE', 'https://yunai.chat'))
...
def _get_model_type_provider_config(model_type: str) -> Dict[str, Any]:
    ...
    if source_lower == 'gemini':
        ...
        api_base = normalize_openai_base_url(
            _resolve_setting(f'{prefix}_API_BASE')
            or _resolve_setting('OPENAI_API_BASE', 'https://yunai.chat')
        )
